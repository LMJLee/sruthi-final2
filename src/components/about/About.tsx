...
const About = ({ icon, title, description, setSelectedPage }: Props) => {
	return (
		<motion.div variants={childVariant} className="mt-5 rounded-md border-2 border-gray-100 px-5 py-16 text-center">
			...
			<AnchorLink className="text-sm font-bold text-primary-500 hover:text-secondary-500" onViewportEnter={() => setSelectedPage(SelectedPage.Contact)} href={`${SelectedPage.Contact}`}>
				<p>Learn More</p>
			</AnchorLink>
		</motion.div>
	);
